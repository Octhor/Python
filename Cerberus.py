...
def cerberus():
    #Find the Xml file present in the folder
    caminho = str(pathlib.Path().absolute())
    for file in os.listdir(caminho):
        if file.endswith("Cerberus.Xml"):
            filename = (os.path.join(caminho, file))
            
    #Initialise the code and get root from Xml
